...
class GoogleController extends GetxController {
  ...
  Future googleLogin() async {
    ...
    final userCrediential = await FirebaseAuth.instance.signInWithCredential(credential);
  ...
  Future<bool> userAlreadyExists(String uid) async {
    ...
    if (documentSnapshot.exists) {
      Map<String, dynamic>? data = documentSnapshot.data() as Map<String, dynamic>?; // Explicit casting
      if (data != null) {
        var userStatus = data['userType'];
        if (userStatus == "host"){
          Get.to(ScreenHostHomePage());
        }
        else{
